...
namespace ProjectPSX.Devices.CdRom {
    public class CD {
        ...
        private readonly Dictionary<string, FileStream> subStreams = new Dictionary<string, FileStream>(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, long> subSectorCounts = new Dictionary<string, long>(StringComparer.OrdinalIgnoreCase);
        private FileStream? discSubStream;
        ...
        private bool TryReadSubchannelQ(int loc, Track currentTrack, out SubchannelQ subQ) {
            ...
            if (subStreams.TryGetValue(currentTrack.file, out FileStream? trackSubStream)
        ...
        private bool TryReadSubchannelQFrame(FileStream stream, int sectorIndex, out SubchannelQ subQ) {
        ...
        private void TryLoadSubchannels(string diskFilename) {
            ...
            foreach (Track track in tracks) {
                ...
                if (!TryOpenSubchannelFile(subPath, out FileStream? stream, out long sectors)) {
                    continue;
                }
        ...
        private static bool TryOpenSubchannelFile(string subPath, out FileStream? stream, out long sectors) {
            stream = null;
            sectors = 0;
            ...
            try {
                stream = new FileStream(subPath, FileMode.Open, FileAccess.Read, FileShare.Read);
